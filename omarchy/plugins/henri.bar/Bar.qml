...
Item {
  ...
  // Plain (not required) so the bar also loads as a plugin bar, where the
  // host injects these through configureBar() after creation.
  ...
  // configureBar() assigns barConfig once; follow later shell.json reloads.
  Connections {
    target: root.shell
    ignoreUnknownSignals: true
    function onBarConfigChanged() { root.barConfig = root.shell.barConfig }
  }
