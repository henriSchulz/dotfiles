...
Item {
  ...
  // ---------------------------------------------------------- spotlight look
  // Wide card, oversized search line, compact rows, pinned high on screen.
  // These tokens proportion the whole launcher — the search glyph sits in the
  // row icon column and the query text on the row label column, so the search
  // line and the results below it share one grid.
  readonly property int spotlightWidth: 580        // card width, in style units
  ...
  readonly property int iconColumnWidth: Style.space(30)
  readonly property int iconColumnLeft: root.rowInset + Style.space(8)
  // "No matches" needs more room than a single row, or the glyph and the
  // message collide in a card sized for one result.
  readonly property int emptyStateHeight: Style.space(96)
  readonly property int labelColumnLeft: root.iconColumnLeft + root.iconColumnWidth + Style.space(6)
  readonly property int gutter: Style.space(19)

  // Left/right padding lives on the header and the row list, so the search
  // line and its underline run edge to edge like Spotlight's.
  property int contentMargin: Style.space(9)
  property int headerHeight: Math.max(Style.space(58), root.searchFontSize + Style.spacing.controlPaddingY * 3)
  property int contentSpacing: Style.spacing.xs
  property int baseRowHeight: Math.max(Style.space(44), Style.font.subtitle + Style.spacing.rowPaddingX * 2)
  property int detailRowHeight: Math.max(Style.space(54), Style.font.subtitle + Style.font.caption + Style.spacing.rowPaddingX * 2)
  ...
  readonly property int gridCellPadTop: Style.space(11)
  readonly property int gridCellPadBottom: Style.space(9)
  readonly property int gridIconGap: Style.space(7)
  ...
  readonly property int gridContentWidth: Math.max(0, resultsViewport.width - root.rowInset * 2)
  ...
  readonly property string searchPlaceholder: {
    ...
    return "Search Omarchy"
  }
  // The app grid needs more width than the result list: the spotlight card
  // keeps its width everywhere else, and widens just enough for the grid.
  ...
  PanelWindow {
    ...
    // Spotlight opens high on the screen and grows downward, rather than
    // sitting dead center.
    readonly property int centeredTop: Math.max(Style.gapsOut, Math.min(
      Math.round(height * root.spotlightTopFraction),
      height - root.cardHeight - Style.gapsOut))
    ...
    BorderSurface {
      ...
      Column {
        ...
        // Spotlight search line: magnifier glyph, oversized query text with a
        // blinking caret, and a full-bleed hairline separating it from results.
        Item {
          id: searchHeader
          width: parent.width
          ...
          Text {
            ...
            anchors.leftMargin: root.iconColumnLeft
            anchors.verticalCenter: parent.verticalCenter
          }

          // Query text and placeholder both start on the row-label column, so
          // the search line and the results below it share one grid. The caret
          // trails the text and parks just left of the placeholder when empty.
          Item {
            id: queryRow
            height: root.searchFontSize + Style.space(6)
            anchors.left: parent.left
            anchors.leftMargin: root.labelColumnLeft
            anchors.right: parent.right
            anchors.rightMargin: root.gutter
            anchors.verticalCenter: parent.verticalCenter

            readonly property int caretGap: Style.space(3)
            ...
            Rectangle {
              ...
              height: Math.round(root.searchFontSize * 1.2)
              radius: width / 2
              // Foreground, not selectedText: themes that pair a strong
              // selection fill with white selected text would make the caret
          ...
          Rectangle {
            ...
            visible: root.mode !== "input" && displayModel.count > 0
          }
        }
          ...
          ListView {
            ...
            delegate: BorderSurface {
              ...
              Image {
                ...
                asynchronous: true
                anchors.left: parent.left
                anchors.leftMargin: root.rowReservedBorderLeft + Style.space(8) + (root.iconColumnWidth - width) / 2
                y: contentColumn.y + labelText.y + (labelText.height - height) / 2
              }
