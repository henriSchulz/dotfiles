...
| `stow/bin/.local/bin/henri-ui-experimental` + `henri-ui-mactahoe-build` | **Experimental macOS mode** for GTK4 apps, switchable in the Control Center (Experiments) or from the shell. `henri-ui-mactahoe-build` clones [MacTahoe](https://github.com/vinceliuice/MacTahoe-gtk-theme) (MIT) into `~/.local/state/henri-ui/vendor/`, patches the few SCSS variables its palette derives from to the active Omarchy theme and compiles it with dart-sass (`npx`, no install) or sassc. `henri-ui-experimental on|off|toggle` rewrites `~/.local/state/henri-ui/experimental.css`, which `stow/gtk/.config/gtk-4.0/gtk.css` imports **first** — so the henri-ui app styles still override it, and off means an empty file. GTK3 has no such user stylesheet, so it gets the whole theme instead: the build also writes `~/.local/share/themes/MacTahoe-Experimental/` and the switch points `gtk-theme` at it, keeping the old name in `experimental.prev-theme`. That is where the mode is actually visible — evince, every GTK3 dialog, Chromium's file chooser — because Files is the only GTK4 app here and `henri-files.css` overrides it on purpose. Quick Look keeps its own look: Sushi starts with `GTK_THEME=HenriQuickLook`, and the variable beats the setting. `henri-ui-gtk-colors` kicks off a detached rebuild when the mode is on, so an Omarchy theme switch carries through. Upstream's `placeholder_text_color` points at the drop-target orange; the build patches it back to hint grey. Nothing is installed into `~/.config/gtk-4.0` by upstream's own installer, which would delete the stow symlink there. GTK reads its stylesheet at startup, so apps change on their next launch |
