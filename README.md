...
| `stow/hypr/.config/hypr/window-mode.lua` | **disabled (not required anywhere), kept for later.** Global window mode: Omarchy tiling or macOS-style floating (new windows float, apps reopen at their last closed spot and cascade, drag to left/right/top edge snaps, `SUPER+ALT+F` zooms, `SUPER+M` minimizes into omadock). Mode + geometry persist in `~/.local/state/henri/` |
| `stow/bin/.local/bin/` | `omarchy-{launch-,}screensaver-themed` — the themed screensaver `henri.idle` shells out to. Without these that plugin is inert. `app-settings` — opens the focused app's settings, called by `henri.active-window`. `agent-session` — starts a coding agent (claude/codex/agy) in a terminal, in a directory, on a Hyprland workspace, optionally with an opening prompt; the voice assistant uses it rather than assembling the terminal command itself, because Omarchy's Lua dispatch does not take a nested quoted `bash -c` and silently runs nothing. It writes a one-shot starter script and hands Hyprland a single path. `agent-guard` — the voice assistant's safety gate: an Antigravity pre-tool hook that lets read-only tools and read-only commands through and stops anything that could write, delete or change until it is confirmed in the assistant card. A `deny` from it means the tool never runs (verified: a denied `rm` left the file on disk). Scoped to the voice session through `HENRI_VOICE=1`, so terminal sessions keep agy's own review. Installed by `install/46-agent-guard.sh` |
...
| `stow/voxtype/.config/systemd/user/voxtype.service.d/` | one drop-in that points voxtype's service at `/usr/lib/voxtype/voxtype-onnx-avx512`, the ONNX build. `/usr/bin/voxtype` is the Whisper-only build and repointing it needs root; the drop-in does the same without it and survives package updates. The engine itself (Parakeet TDT v3 int8 — ~1 s per recording instead of Whisper `small`'s flat ~10 s) is set by `install/45-voxtype.sh`, because `config.toml` is rewritten by `voxtype config set` |
...
| `stow/bin/.local/bin/henri-ui-experimental` | **Experimental macOS mode** for the shell, switchable in the Control Center (Experiments) or from a terminal. It flips one flag in `henri-ui/Experimental.js`, whose table of macOS Tahoe values (radii, control heights, read off MacTahoe's `_variables.scss`) `Motion.js` lays over its own tokens while the flag is set. Every plugin imports `Motion.js`, so the whole shell changes shape at once; `henri-ui-sync` restarts it a moment later, which is required because already-loaded henri-ui components stay cached. It also turns the shell to glass: surfaces drop from the theme's 0.97 to 0.65 (menus 0.75), `HUi.GlassSheen` puts Tahoe's lit top edge on them, Control Center tiles become frosted instead of a dark wash, and `looknfeel.lua` blurs the shell's popup layers (`omarchy-keyboard-panel`, `omarchy-menu`, `henri-osd`) — permanently, since blur behind a near-opaque surface is invisible anyway. Secondary text goes to alpha 0.85 there: measured 3.3:1 at the usual 0.65 over glass on black content, 5.0:1 at 0.85. It also swaps the icon theme to MacTahoe and parks `hicolor/scalable/apps/org.gnome.Nautilus.svg` (my own Files icon, there because Yaru-blue ships none) while it runs — hicolor is searched before the active theme, so it would beat MacTahoe's Finder face in the dock. Motion is deliberately left out — MacTahoe's own curves are Material Design, not macOS. The mirrored copies in the plugins that vendor henri-ui always get the flag forced off, so a toggle never dirties those public repos. A first attempt aimed at GTK instead and was dropped: Files is the only GTK4 app here and `henri-files.css` overrides it on purpose, so it moved 1.7 % of pixels |
| `stow/bin/.local/bin/henri-macos-icons` | Installs the [MacTahoe icon theme](https://github.com/vinceliuice/MacTahoe-icon-theme) (GPL-3.0) into `~/.local/share/icons` through upstream's own installer, which also builds the icon caches. 220 MB installed, source checkout in the state directory — deliberately not in this repo: the size, and a GPL-3.0 icon set sitting inside a public repo of my own. `--remove` takes all three variants back out |
| `omarchy/omadock.json` + `omarchy/shell.json` | The dock, enabled and set up as the macOS one: centre aligned, wave magnification, launch bounce, running dots, a Downloads stack behind the separator, `iconSize` 56 because omadock's auto size follows the bar (~30 px) and macOS sits near 64, background at 0.65 with a blur layer rule for `omadock` in `looknfeel.lua`. One thing is patched in the plugin itself and will be lost when omadock updates: `Dock.qml`'s `zoomPeak` 1.22 → 1.6, because 1.22 barely magnifies and the wave is the signature of that dock. No Trash — omadock has none |
| `stow/bin/.local/bin/henri-gtk-preview` | Screenshots GTK widgets (`--gtk3` for the GTK3 sheet) or a whole app (`--app nautilus`) while the desktop is in use, without anything appearing on screen. A throwaway headless output takes the window. Two traps it documents: `hyprctl keyword windowrule[v2]` does **nothing** here — the Lua config has no legacy parser and hyprctl says so on stdout with exit 0, so the window quietly opens on the real screen; rules must ride along with the launch, `hyprctl eval "hl.exec_cmd(cmd, { workspace = '<id> silent' })"`. And `grim -o HEADLESS-n` returns an empty desktop, because Hyprland composites only layer surfaces (bar, wallpaper) there, never windows — capture the toplevel with `grim -T <stableId>`. GTK4 also has no offscreen window any more, so its sheet snapshots itself in-process once mapped; GTK3 still has `Gtk.OffscreenWindow` and needs none of this |
...
| `omarchy/plugins/henri.assistant/` | own plugin: speak a prompt into a Claude Code session. `SUPER+A` opens a pill in the middle of the screen and starts listening; the same key ends the recording and the transcript goes straight into a new Claude Code session in a terminal (via `agent-session`), then the card closes. The recording runs with `--file` and `--no-osd`, so the text never goes through the keyboard or the clipboard. It used to run an Antigravity agent inside the card — conversation, thinking dots, a permission window per tool — which is in the history if it is ever wanted back |
| `omarchy/plugins/henri.app-switcher/` | own plugin: macOS-style Cmd+Tab on **Alt+Tab** — hold Alt for a row of app icons (most recently used first, one icon per app, not per window), Tab picks, releasing Alt switches, a quick tap goes straight back to the previous app. Fn is not bindable on this machine: the keyboard controller handles it and Fn+Tab sends the same keycode as Tab alone (measured), while Alt sits exactly where ⌘ sits on a Mac. Replaces Omarchy's `ALT+TAB` window cycling. Keys travel as Hyprland events (`custom>>app-switcher …`), the release bind needs `transparent = true` |
...
| `omarchy/plugins/henri.wallpaper/` | own plugin: macOS-style wallpaper picker (SUPER+CTRL+SPACE, double click on the desktop, IPC `omarchy-shell wallpaper toggle`) for `~/Pictures/Wallpaper`; a click sets the image, "Für alle Themes verwenden" keeps it across theme changes. Backed by `henri-wallpaper` (`stow/bin`), which records the choice in `~/.config/omarchy/global-wallpaper` (machine-local, not in the repo) and caches 16:10 thumbnails in `~/.cache/henri-wallpaper` |
...
| `omarchy/plugins/henri.control-center/` | own plugin: macOS-style Control Center popup in the bar (Wi-Fi, Bluetooth, AirDrop via LocalSend, Focus/DND, Night Shift, Stay Awake, display + sound sliders, Now Playing); round icon toggles, label opens an in-popup detail page (Wi-Fi networks with inline password entry and expandable advanced options — live ping/throughput/IP, band, DNS, QR share, speed test; Bluetooth devices; sound output), Esc goes back; a Tiling tile switches the active workspace's layout (dwindle/scrolling); animated page slides, staggered open, springy toggles; the display tile expands to text size, scale presets and monitors; hosts `omaplug`'s panel ("Plugins verwalten"), which is deliberately kept off the bar. An **Experiments** page carries switches for things being tried out — first of them the experimental macOS mode for GTK apps, driven by `henri-ui-experimental` so the switch and the CLI can never disagree |
