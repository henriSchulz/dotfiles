...
| `stow/hypr/.config/hypr/looknfeel.lua` | +7 lines: `rounding = 12`; loads `window-mode.lua` |
| `stow/hypr/.config/hypr/window-mode.lua` | global window mode, switched from the control center: Omarchy tiling or macOS-style floating (new windows float, apps reopen at their last closed spot and cascade, drag to left/right/top edge snaps, `SUPER+ALT+F` zooms, `SUPER+M` minimizes into omadock). Mode + geometry persist in `~/.local/state/henri/` |
...
| `omarchy/plugins/henri.control-center/` | own plugin: macOS-style Control Center popup in the bar (Wi-Fi, Bluetooth, AirDrop via LocalSend, Focus/DND, Night Shift, Stay Awake, display + sound sliders, Now Playing); round icon toggles, label opens an in-popup detail page (Wi-Fi networks with inline password entry and expandable advanced options — live ping/throughput/IP, band, DNS, QR share, speed test; Bluetooth devices; sound output), Esc goes back; a "Fenster" tile switches the window mode (Tiling / Freie Fenster) and, while tiling, shows the active workspace's layout (dwindle/scrolling) as an inset sub-option; animated page slides, staggered open, springy toggles; the display tile expands to text size, scale presets and monitors; hosts `omaplug`'s panel ("Plugins verwalten"), which is deliberately kept off the bar |
