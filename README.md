...
| `stow/bin/.local/bin/henri-ui-experimental` + `henri-ui-mactahoe-build` | **Experimental macOS mode** for GTK4 apps, switchable in the Control Center (Experiments) or from the shell. `henri-ui-mactahoe-build` clones [MacTahoe](https://github.com/vinceliuice/MacTahoe-gtk-theme) (MIT) into `~/.local/state/henri-ui/vendor/`, patches the few SCSS variables its palette derives from to the active Omarchy theme and compiles it with dart-sass (`npx`, no install) or sassc. `henri-ui-experimental on|off|toggle` only rewrites `~/.local/state/henri-ui/experimental.css`, which `stow/gtk/.config/gtk-4.0/gtk.css` imports **first** — so the henri-ui app styles still override it, and off means an empty file. Nothing is installed into `~/.config/gtk-4.0` by upstream's own installer, which would delete the stow symlink there. GTK reads its stylesheet at startup, so apps change on their next launch |
| `stow/bin/.local/bin/henri-gtk-preview` | Renders a sheet of GTK4 widgets to a PNG for checking GTK styling while the desktop is in use: the window has to be mapped or GTK renders nothing, so Hyprland parks it on a hidden special workspace without focus |
