...
| `omarchy/plugins/henri.assistant/` | own plugin: the voice assistant. SUPER + right Ctrl opens the dictation pill grown into a card — the transcript lands in a composer, ↵ sends it to Antigravity (`agy --input-format stream-json`, one warm process per session so follow-ups keep their context), the answer streams in as Markdown. The recording runs with `--file` and `--no-osd`, so the text never goes through the keyboard or the clipboard. `omarchy-shell henri-assistant ask "…"` asks without the microphone |
