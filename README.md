...
| `omarchy/omadock.json` + `omarchy/shell.json` | The dock, enabled and set up as the macOS one: centre aligned, launch bounce, running dots, a Downloads stack behind the separator, `iconSize` 56 because omadock's auto size follows the bar (~30 px) and macOS sits near 64, background at 0.65 with a blur layer rule for `omadock` in `looknfeel.lua`. `hoverEffect` is `off`: the icons do not grow under the cursor — the one macOS behaviour deliberately not copied, tooltips still work. Nothing is patched inside the plugin, so an omadock update carries cleanly. No Trash — omadock has none |
