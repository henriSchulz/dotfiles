...
| `stow/bin/.local/bin/henri-gtk-preview` | Renders a sheet of widgets to a PNG for checking GTK styling while the desktop is in use. GTK4 has no offscreen window any more and draws nothing until the compositor maps it, so a throwaway headless output is created, the window opens there `nofocus`, and the picture is taken in-process — pointing `grim` at that output returns an empty desktop, because Hyprland composites only layer surfaces (bar, wallpaper) there, never windows. `--gtk3` is the easy half: `Gtk.OffscreenWindow` still exists, no output and no rules needed |
