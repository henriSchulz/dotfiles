...
| `omarchy/plugins/henri.assistant/` | own plugin: the voice assistant. `SUPER+A` opens a card in the middle of the screen and starts listening; `↵` ends the recording and sends the transcript straight to Antigravity (`agy --input-format stream-json`, one warm process per session so follow-ups keep their context), the answer streams in as Markdown. The recording runs with `--file` and `--no-osd`, so the text never goes through the keyboard or the clipboard. Not on `SUPER` + right Ctrl: right Ctrl is itself a modifier, so the modifier state changes mid-chord and Hyprland re-matched the dictation bind underneath, whose toggle stopped the recording a tenth of a second after it started. `omarchy-shell henri-assistant ask "…"` asks without the microphone |
