...
| `omarchy/plugins/henri.control-center/` | own plugin: macOS-style Control Center popup in the bar (Wi-Fi, Bluetooth, AirDrop via LocalSend, Focus/DND, Night Shift, Stay Awake, display + sound sliders, Now Playing); round icon toggles, label opens an in-popup detail page (Wi-Fi networks with inline password entry, Bluetooth devices, sound output), Esc goes back; the display tile expands to text size, scale presets and monitors; hosts `omaplug`'s panel ("Plugins verwalten"), which is deliberately kept off the bar |
