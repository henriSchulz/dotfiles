.pragma library
// Henri UI tokens — SINGLE SOURCE OF TRUTH for every QML plugin.
// Never copy this file into a plugin; import it by absolute path:
//   import "file:///home/henri/.local/share/henri-ui/Motion.js" as Motion
// Change a value here + reload the shell → every plugin follows.

// Durations (ms)
var instant = 90
var fast = 160
var base = 240
var slow = 380
var slower = 520

function exit(ms) { return Math.round(ms * 0.7) }

// Bezier curves for `easing.type: Easing.BezierSpline` (Qt wants the end point 1,1 appended)
var easeOut = [0.22, 1, 0.36, 1, 1, 1]
var easeInOut = [0.45, 0, 0.15, 1, 1, 1]
var easeExit = [0.4, 0, 0.7, 0.2, 1, 1]

// Press feedback
var pressScale = 0.97
// Enter/exit scale for menus and popovers
var menuFromScale = 0.96
var popoverFromScale = 0.95
var exitToScale = 0.98
var menuOffsetY = -4

// List stagger
var staggerStep = 15
var staggerMax = 10
function stagger(index) { return Math.min(index, staggerMax) * staggerStep }

// Shape (px) — radii per surface, see SKILL.md §4
var radiusPanel = 14
var radiusPopover = 10
var radiusControl = 8
var radiusRow = 6
var radiusChip = 5

// Spring presets for HUi.SpringValue { preset: Motion.smooth } (SwiftUI response/dampingRatio)
var smooth = { response: 0.35, dampingRatio: 1.0 }
var snappy = { response: 0.40, dampingRatio: 0.85 }
var gentle = { response: 0.50, dampingRatio: 1.0 }
var bouncy = { response: 0.45, dampingRatio: 0.75 }

// Kinetic scrolling
var flickDeceleration = 1500
var maximumFlickVelocity = 4000
