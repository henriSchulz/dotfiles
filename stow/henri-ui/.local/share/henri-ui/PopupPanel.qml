...
// HENRI-UI: drop-in replacement for the shell's Ui/KeyboardPanel — same API,
// same focus/positioning/dismissal logic (copied from Omarchy, MIT), but the
// card appears through HUi.Reveal (scale + fade out of the bar button,
// spring, exit faster than enter), uses henri-ui radii, and switching between
// bar popups is instant like the macOS menu bar. Swap `KeyboardPanel {` for
// `HUi.PopupPanel {` — nothing else changes. `kind`: "popover" (default) or
// "panel" (bigger surfaces, gentler spring).
//
...
PanelWindow {
  ...
  visible: open || reveal.visible || popoutSwitching
  color: "transparent"
  ...
  // The card grows out of the bar edge it hangs from.
  ...
  Reveal {
    ...
    // KeyboardPanel already handles Esc (PanelKeyCatcher) and outside clicks
    // (dismissArea + twins), with bar-click forwarding the Reveal can't do.
    closeOnEscape: false
    closeOnOutsideClick: false
    // Keyboard focus belongs to focusTarget, not the Reveal.
