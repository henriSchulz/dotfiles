...
// HENRI-UI: drop-in replacement for the shell's Ui/PopupCard — same API,
// same PopupWindow anchoring, HyprlandFocusGrab dismissal, triggerMode,
// fitted*/capped* helpers and owner/coordinator logic (copied from Omarchy,
// MIT). Only the card's appearance changed: instead of a flat 140 ms opacity
// fade it appears through HUi.Reveal (fade + scale out of the bar edge it
// hangs from, spring, exit faster than enter) and uses henri-ui radii.
// Swap `PopupCard {` for `HUi.PopupCard {` — nothing else changes.
//
// Added (optional): `kind` — "popover" (default), "menu" (tray/context
// menus: smaller scale, drops 4 px out of the bar) or "panel" (big surfaces).
PopupWindow {
  id: root
  ...
  // henri-ui: menu | popover | panel — choreography + radius.
  ...
  // Stays mapped until the Reveal's exit has finished.
  visible: open || reveal.visible
  color: "transparent"
  ...
  // The card grows out of the bar edge it hangs from; menus also drop a few
  // px out of it (Motion.menuOffsetY is negative = away from a top bar).
  readonly property int revealOrigin: barPos === "bottom" ? Item.Bottom
    : barPos === "left" ? Item.Left : barPos === "right" ? Item.Right : Item.Top
  readonly property real revealOffset: kind === "menu" ? Math.abs(Motion.menuOffsetY) : 0
  ...
  Reveal {
    ...
    fromX: root.barPos === "left" ? -root.revealOffset : root.barPos === "right" ? root.revealOffset : 0
    fromY: root.barPos === "bottom" ? root.revealOffset
      : (root.barPos === "left" || root.barPos === "right") ? 0 : -root.revealOffset
    open: root.open
    // PopupCard already dismisses itself (HyprlandFocusGrab above, owner.close()).
    closeOnEscape: false
